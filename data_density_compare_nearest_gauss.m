...
%% This is the first line: the satellite data density using gauss resample
x0=(1:length(datevec))';
...
%% this is the second line of the figure: the satellite data density using nearest resample
...
data_density_satellite_hourly_season_nearest=nansum(data_density_satellite_hourly,3);
satellite_den_hourly_nearest{j}=data_density_satellite_hourly_season_nearest;
clear data_density_satellite_hourly
...
if i==1
    ylabel('Nearest neighbour','FontSize',20)
end
...
m_pcolor(lons,lats,eval(['satellite_den_',data_name{j},'_nearest{i}./length(x',num2str(i-1),')']));
hold on
m_contour(lons,lats,seaice_conc_cdr_climitology{i},[0.8 0.8],'red','LineWidth',2);
hold off
caxis([-40 10])
